...
The final process involves validating the intial hypothesis by comparing NSAIDs (the backlash drug) against a control drug (statins/lipid -regulators)

Which leads to a different hypothesis: 
if the drop was just people afraid to commute to GPs due to lockdown restrictions both drug classes should drop, if the WFH backlash is in fact real, NSAIDs should rebound harder or behave differently than Statins in the winter. 

```{r load-statin-data, message=FALSE, warning=FALSE, echo=FALSE}
# Part 8 - Control Group Analysis 

# to validate our findings, we compare NSAIDs to a "Control" drug: 
# lipid-regulating drugs (BNF chapter 2.12, e.g. Statins)
# These are chronic medications with no seasonal link and no connection to WFH pain 

# 1. Create the Statin Summary from the raw data
statin_summary <- raw_data_cleaned %>%
  # Filter for Lipid-Regulating Drugs (BNF starts with '0212')
  filter(str_starts(bnf_item_code, "0212")) %>%
  # Format the date
  mutate(Month = ymd(paste0(paid_date_month, "01"))) %>%
  # Group and count
  group_by(Month) %>%
  summarise(TotalItems = n()) %>%
  arrange(Month) %>%
  # Label it as the Control group
  mutate(Drug_Type = "Control: Statins")

# 2. Get our existing NSAID Summary ready to join
nsaid_summary_labelled <- summary_data %>%
  mutate(Drug_Type = "Target: NSAIDs/MSK")

# 3. Combine them into one dataset for plotting
comparison_data <- bind_rows(nsaid_summary_labelled, statin_summary)

# make sure to use the scales package 
# this package allows for the automatic formatting of axis labels with commas making prescription numbers easier to read
library(scales)

# Part 9. Creating the final comparison plot

# step 1. Create the Plot and store it in 'final_plot'
# this essentially ensures there is no text dump at the end 
final_plot <- ggplot(comparison_data, aes(x = Month, y = TotalItems, color = Drug_Type)) +
  
  # Lines and Points
  geom_line(linewidth = 1) +
  geom_point() +
  
  # Lockdown Marker, creates a red line to mark start of COVID lockdown
  geom_vline(xintercept = as.Date("2020-03-23"), linetype = "dashed", color = "red") +
  
  # Facet the plots
  facet_wrap(~Drug_Type, scales = "free_y", ncol = 1) +
  
  # Formatting
  scale_y_continuous(labels = scales::comma) +
  scale_x_date(date_breaks = "3 months", date_labels = "%b %y") +
  scale_color_manual(values = c("gray40", "#0072B2")) + 
  theme_minimal() +
  theme(legend.position = "none") +
  
  # Labels
  labs(
    title = "The 'Backlash' Test: NSAIDs vs. Control Group",
    subtitle = "Comparing Pain Meds (Target) against Statins (Control) during Lockdown",
    y = "Total Monthly Prescriptions",
    x = "Month"
  )

# 2. Explicitly print the plot
# This ensures only the image renders, not the text structure.
final_plot
```
Conclusion:
The overall analysis successfully addressed the conflicting forces proposed In the introduction: the “increased physical strain” of remote work (Guler et al., 2021; El Kadri Filho and Roberto de Lucca, 2022) versus the “decreased access” to primary (Welsh et al., 2023). The data reveals that the decreased access force initially dominated, causing a majority, V-shaped drop in NSAID and MSK prescriptions across all of Scotland during the first lockdown. However, the subsequent behaviour of the data supports the WFH backlash hypothesis, Unlike the control group (statins), which fluctuated without a clear pattern, NSAID/MSK prescriptions demonstrated a steady recovery, regaining their season winter peak by late 2020. This robust recovery aligns with findings by Welsh et al. (2023), who noted that while total consultations remained low, there was a specific “upturn” in analgesic prescribing from 2020. Additionally, the urban-rural analysis (Scottish Government, 2022) highlighted that while urban centres faced limited GP access, remote rural areas remained more resilient.
